...
train_dataset = datasets.Country211(root='./data', split='train', download=True, transform=transform)
test_dataset = datasets.Country211(root='./data', split='test', download=True, transform=transform)

batch_size = 32
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
test_loader = DataLoader(test_dataset, batch_size=batch_size)

check_epochs = np.arange(0, 101, 3)
...
for iteration in range(n_repeat):
    ...
    model.fc = nn.Linear(model.fc.in_features, 211)
    # for param in model.conv1.parameters():
    #     param.requires_grad = False
    # for param in model.bn1.parameters():
    #     param.requires_grad = False
...
results.to_csv(f"country_full_dataset_trainings_resnet152_20230915.csv", index=False)
