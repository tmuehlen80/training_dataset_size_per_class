...
#subset_sizes = np.arange(30000, 45000 + 1, 5000).tolist()
...
for n_max in subset_sizes:
    ...
    doe = create_constrained_mixture_design(n_classes, n_repeat, n_max, 5000, n_optim=500000, n_batch_size=1000)
    ...
    results.to_csv(f"Cifar10_acc_subsets_thomas_batch_size_512_mixture_design_maximin_subsetsize_{n_max}_val_20230708.csv", index=False)
...
for i, c in enumerate(classes):
    results[c] = subsets_collected[:, i]
results["epochs_trained"] = epochs_trained
results.to_csv("Cifar10_acc_subsets_thomas_batch_size_512_mixture_design_maximin_val_20230708.csv", index=False)
